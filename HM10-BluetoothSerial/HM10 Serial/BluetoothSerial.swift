...
final class BluetoothSerial: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func sendMessageToDevice(_ message: String) {
        ...
        while (i < message.count){
            ...
            if i+portion < message.count {
                end = message.index(message.startIndex, offsetBy: i+portion)
            }else{
                end = message.index(message.endIndex, offsetBy: 0)
            }
            let range = start..<end
            let subString = message[range]  // play
            print("\n")
            print(subString)
